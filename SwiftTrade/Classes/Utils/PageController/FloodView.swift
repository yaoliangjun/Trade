...
class FloodView: ProgressView {
    ...
    override func willMove(toSuperview newSuperview: UIView?) {
        ...
        margin = viewHeight * 0.15
        radius = (viewHeight - margin * 2.0) / 2.0
    }
    ...
    override func draw(_ rect: CGRect) {
        ...
        ctx?.translateBy(x: 0.0, y: viewHeight)
        ctx?.scaleBy(x: 1.0, y: -1.0)
        ctx?.addArc(center: CGPoint(x: startX + radius, y: viewHeight / 2.0), radius: radius, startAngle: CGFloat(Double.pi / 2), endAngle: CGFloat(Double.pi / 2) * 3, clockwise: false)
        ...
        ctx?.addArc(center: CGPoint(x: endX - radius, y: viewHeight / 2.0), radius: radius, startAngle: CGFloat(-Double.pi / 2), endAngle: CGFloat(Double.pi / 2), clockwise: false)
